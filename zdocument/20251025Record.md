...
优化文件@scripts/zh_video_translate_deepseek.py实现支持`多人模式`的转录，通过重构文件@src/asr_funasr.py实现。入参增加参数`--multi`，默认是1 个人，传参为 2 的 2 个人，3 是 3 个人，4 是 4 人，`n`是`多人`。Swift文件@zrecord/RemoteFunASRService.swift里已经实现了`多人模式`的转录逻辑，参考其实现逻辑。
